...
class ReportController
{
    ...
    public function all_reports_for_timeframe( $from, $to )
    {
        $stmt = $this->pdo->prepare(
            ...
            ' r.created_at >= :from_date AND r.created_at < :to_date;'
        );
        $stmt->execute(
            [
            'from_date' => $from,
    ...
    {
        $stmt = $this->pdo->prepare(
            'INSERT INTO reports VALUES(NULL,:session_id,:num_chars,:num_words,' .
            ':num_sentences,:num_palindromes,:freq_chars,' .
            ':percentage_chars,:avg_word_length,:avg_num_of_words_in_sentence,' .
            ':top_10_mu_words,:top_10_longest_words,:top_10_shortest_words,' .
            ':top_10_longest_sentences,:top_10_shortest_sentences,' .
            ':top_10_longest_palindromes,:is_text_palindrome,:reversed_text,' .
